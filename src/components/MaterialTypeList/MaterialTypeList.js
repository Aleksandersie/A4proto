    ...
    <Card style={{ textAlign: "center"}} className="pb-3 mt-3" >
      <Card.Header>
        ...
        <div className="d-flex justify-content-evenly">{materialList.intPrintMaterial.map((material) => (
            <Card style={{minWidth:180,width:230}} className="mt-3 pb-3" key={material.id}>
                <Card.Header style={{ fontWeight: 500 }}>{material.name}</Card.Header>
                ...
                <Button variant={material.id=== materialList.selectedIntPrintMaterial.id? 'danger':"warning"}
                        style={{width:150}}
                        className="m-auto"
